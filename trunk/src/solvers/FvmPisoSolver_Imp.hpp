        ...
        CGeoVector<Real> CFvmPisoSolver<Real>::gradient(varMap& var, varMap& varf, const Integer aControlVolumeId)
        {
            ...
            for (Integer j = 0; j < m_fvmMesh.controlVolume(aControlVolumeId).nbFaces(); ++j)
            {
                ...
                if (m_fvmMesh.face(aFaceId).hasPair())
                {
                    ...
                    v = var.at(aControlVolumeId) + var.at(aNeighborId); v *= 0.5;
                ...
                for (Integer j = 0; j < m_fvmMesh.controlVolume(aControlVolumeId).nbFaces(); ++j)
                {
                    ...
                    if (m_fvmMesh.face(aFaceId).hasPair())
                    {
                        ...
                        if (m_fvmMesh.controlVolume(aNeighborId).containsFace(aFaceId))
                        {
                            ...
                            dens += m_dens.at(aNeighborId); dens *= 0.5;
                            visc += m_visc.at(aNeighborId); visc *= 0.5;
                    ...
                    else
                    {
                        ...
                        bu[anIndexP] += visc * m_uf.at(aFaceId) * area / dist / volume;
                        bv[anIndexP] += visc * m_vf.at(aFaceId) * area / dist / volume;
                        bw[anIndexP] += visc * m_wf.at(aFaceId) * area / dist / volume;

                        // Convection
                        bu[anIndexP] += -dens * m_uf.at(aFaceId) * area * flux * aNormal.x() / volume;
                        bv[anIndexP] += -dens * m_vf.at(aFaceId) * area * flux * aNormal.y() / volume;
                        bw[anIndexP] += -dens * m_wf.at(aFaceId) * area * flux * aNormal.z() / volume;
                    }
                }
                ...
                for (Integer j = 0; j < m_fvmMesh.controlVolume(aControlVolumeId).nbFaces(); ++j)
                {
                    ...
                    if (m_fvmMesh.face(aFaceId).hasPair())
                    {
                        ...
                        if (m_fvmMesh.controlVolume(aNeighborId).containsFace(aFaceId))
                        {
                            ...
                            apj += m_ap.at(aNeighborId); apj *= 0.5;
                    ...
                    else
                    {
                        ...
                        else if (m_fvmMesh.face(aFaceId).boundaryType() == BT_INLET_FLOW)
                        {
                            ...
                            m_flux[aFaceId] = -(m_uf.at(aFaceId) * aNormal.x() + m_vf.at(aFaceId) * aNormal.y() + m_wf.at(aFaceId) * aNormal.z()) * area;
                        }
                        ...
                        else if (m_fvmMesh.face(aFaceId).boundaryType() == BT_OUTLET)
                        {
                            ...
                            m_uf[aFaceId] = -m_flux.at(aFaceId) / area * aNormal.x();
                            m_vf[aFaceId] = -m_flux.at(aFaceId) / area * aNormal.y();
                            m_wf[aFaceId] = -m_flux.at(aFaceId) / area * aNormal.z();
                        }
                    }
                }
            }
               ...
               Integer aControlVolumeId = m_fvmMesh.controlVolumeId(i);
                ...
                for (Integer j = 0; j < m_fvmMesh.controlVolume(aControlVolumeId).nbFaces(); ++j)
                {
                    ...
                    else
                    {
                        ...
                        {
                            m_flux[aFaceId] += area / (apj * dist) * (m_pf.at(aFaceId) - m_p.at(aControlVolumeId));
                        }
                        else if (m_fvmMesh.face(aFaceId).boundaryType() == BT_OUTLET)
                        {
