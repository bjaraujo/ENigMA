...
namespace ENigMA
{
    ...
    namespace mesh
    {
        ...
        CMshTetrahedronMesher<Real>::CMshTetrahedronMesher() : m_timeInterval(1), m_dataInterval(100), m_bStop(false), onUpdate(nullptr), m_previousNbElements(0)
        {
        ...
        bool CMshTetrahedronMesher<Real>::pairEdges(SAdvancingFrontTriangle& anAdvTriangle1, SAdvancingFrontTriangle& anAdvTriangle2)
        {
            ...
            for (Integer k = 0; k < 3; ++k)
            {
                ...
                for (Integer l = 0; l < 3; l++)
                {
                    ...
                    if ((anAdvTriangle1.nodeId[(k + 0) % 3] == anAdvTriangle2.nodeId[(l + 0) % 3] && anAdvTriangle1.nodeId[(k + 1) % 3] == anAdvTriangle2.nodeId[(l + 1) % 3]) ||
                        (anAdvTriangle1.nodeId[(k + 0) % 3] == anAdvTriangle2.nodeId[(l + 1) % 3] && anAdvTriangle1.nodeId[(k + 1) % 3] == anAdvTriangle2.nodeId[(l + 0) % 3]))
                    {
        ...
        void CMshTetrahedronMesher<Real>::adjustConnectivity(std::vector<Integer>& sTriangles)
        {
            ...
            try
            {
                ...
                for (Integer i = 0; i < static_cast<Integer> (m_anAdvFront.size()); ++i)
                {
                    ...
                    if (anAdvTriangle.nodeId[0] == anAdvTriangle.nodeId[1] ||
                        anAdvTriangle.nodeId[1] == anAdvTriangle.nodeId[2] ||
                        anAdvTriangle.nodeId[2] == anAdvTriangle.nodeId[0])
                    {
                    ...
                    if (anAdvTriangle.neighborId[0] >= static_cast<Integer>(m_anAdvFront.size()) ||
                        anAdvTriangle.neighborId[1] >= static_cast<Integer>(m_anAdvFront.size()) ||
                        anAdvTriangle.neighborId[2] >= static_cast<Integer>(m_anAdvFront.size()))
                    {
                        throw std::out_of_range("Connectivity is out of range!");
                    }

                    if (m_anAdvFront[anAdvTriangle.neighborId[0]].remove ||
                        m_anAdvFront[anAdvTriangle.neighborId[1]].remove ||
                        m_anAdvFront[anAdvTriangle.neighborId[2]].remove)
                    {
                        ...
                        if (anAdvTriangle.neighborId[0] == std::numeric_limits<Integer>::max() ||
                        ...
                        for (Integer j = 0; j < anElement.nbFaceIds(); ++j)
                        {
                            ...
                            if (aFace.hasPair())
                            {
                                ...
                                if (newTriangleIds.find(aNeighborId) != newTriangleIds.end())
                                {
                                    ...
                                    {
                                        if (aNeighbor.nodeId(k) != aFace.nodeId(0) &&
                                            aNeighbor.nodeId(k) != aFace.nodeId(1))
                                        {
                ...
                for (Integer i = firstIndex; i < static_cast<Integer>(m_anAdvFront.size()); ++i)
                {
                    ...
                    if (aNodeId4 == aNodeId5 && aNodeId5 == aNodeId6)
                    {
                        ...
                        if (aNewTetrahedron.volume() > aTolerance * aTolerance * aTolerance)
                        {
                            ...
                            if (this->triangleOk(anAdvTriangle, aNode1, aNode2, aNode4, sTriangles, aTolerance) &&
                                this->triangleOk(anAdvTriangle, aNode2, aNode3, aNode4, sTriangles, aTolerance) &&
                                this->triangleOk(anAdvTriangle, aNode3, aNode1, aNode4, sTriangles, aTolerance) &&
                                !this->tetrahedronContainsNode(aNode1, aNode2, aNode3, aNode4, wNodeId, sNodes, aTolerance))
                            {
        ...
        void CMshTetrahedronMesher<Real>::flipEdges23(const Real aTolerance)
        {
            ...
            for (Integer i = 0; i < m_volumeMesh.nbFaces(); ++i)
            {
                ...
                if (aFace.hasPair())
                {
                    ...
                    if (std::min(q3, std::min(q4, q5)) > std::min(q1, q2) &&
                        aTetrahedron3.volume() > aTolerance * aTolerance * aTolerance &&
                        aTetrahedron4.volume() > aTolerance * aTolerance * aTolerance &&
                        aTetrahedron5.volume() > aTolerance * aTolerance * aTolerance)
                    {
            ...
            for (Integer i = 0; i < m_volumeMesh.nbElements(); ++i)
            {
                ...
                for (Integer j = 0; j < anElement.nbFaceIds(); ++j)
                {
                    ...
                    for (Integer k = 0; k < aFace.nbNodeIds(); ++k)
                    {
                        ...
                        if (bBoundaryNode[aFace.nodeId((k + 0) % 3)] &&
                            bBoundaryNode[aFace.nodeId((k + 1) % 3)])
                            continue;
                        ...
                        if (sElementIds.size() == 3)
                        {
                            ...
                            if (std::min(q4, q5) > std::min(q1, std::min(q2, q3)) &&
                                aTetrahedron4.volume() > aTolerance * aTolerance * aTolerance &&
                                aTetrahedron5.volume() > aTolerance * aTolerance * aTolerance)
                            {
