...
FvmVof::FvmVof()
{
    ...
    this->ui = new Ui_FvmVof;
    this->ui->setupUi(this);

    this->setWindowTitle("Fvm VOF");
...
void FvmVof::solve()
{
    ...
    CGeoCoordinate<double> aVertex1(+0.00, +0.00, -0.2);
    CGeoCoordinate<double> aVertex2(+1.00, +0.00, -0.2);
    CGeoCoordinate<double> aVertex3(+1.00, +1.00, -0.2);
    CGeoCoordinate<double> aVertex4(+0.00, +1.00, -0.2);
    CGeoCoordinate<double> aVertex5(+0.00, +0.00, +0.2);
    CGeoCoordinate<double> aVertex6(+1.00, +0.00, +0.2);
    CGeoCoordinate<double> aVertex7(+1.00, +1.00, +0.2);
    CGeoCoordinate<double> aVertex8(+0.00, +1.00, +0.2);
    ...
    for (Integer i = 0; i < aFvmMesh.nbControlVolumes(); ++i)
    {
        ...
        if (aControlVolume.centroid().y() < 0.5)
    ...
    double rho1 = 1000.0; // density
    double mu1 = 1.0; // dynamic viscosity

    double g = -9.8;
    aVofSolver.setGravity(0.0, g, 0.0);

    aVofSolver.setMaterialProperties(rho0, mu0, rho1, mu1);

    std::vector<Integer> sWallIds;
    ...
    for (Integer i = 0; i < aFvmMesh.nbFaces(); ++i)
    {
        ...
        if (aFace.centroid().x() < 1E-6 ||
            aFace.centroid().x() > (1.0 - 1E-6) ||
            aFace.centroid().y() < 1E-6 ||
            aFace.centroid().y() > (1.0 - 1E-6))
        {
            sWallIds.push_back(aFaceId);
        }

    }

    aVofSolver.setBoundaryVelocity(sWallIds, BT_WALL_NO_SLIP, 0.0, 0.0, 0.0);
    aVofSolver.setBoundaryPressure(sWallIds, BT_WALL_NO_SLIP, 0.0);
    aVofSolver.setBoundaryGamma(sWallIds, BT_WALL_NO_SLIP, 0.0);
    ...
    for (Integer ii = 0; ii < nIter; ++ii)
    {
        ...
        for (Integer i = 0; i < aFvmMesh.nbControlVolumes(); ++i)
        {
            ...
            sScalars->SetTuple1(aControlVolumeId, aVofSolver.p(aControlVolumeId));
